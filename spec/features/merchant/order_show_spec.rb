...
RSpec.describe 'Merchant Order Show Page' do
  describe 'As a Merchant' do
    before :each do
      ...
      @m_address = @m_user.addresses.create!(street: "Boozy Q", city: "Running", state: "Outta", zip: 39847)
      ...
      @order_1 = @m_user.orders.create!(address_id: @m_address.id, status: "pending")
      @order_2 = @m_user.orders.create!(address_id: @m_address.id, status: "pending")
    ...
    it 'I can see order information with only my order items' do
      ...
      expect(page).to have_content(@order_2.address.city)
      expect(page).to have_content(@order_2.address.state)
      expect(page).to have_content(@order_2.address.zip)
