...
RSpec.describe 'User Show Page' do
  describe 'As an Admin' do
    before :each do
      ...
      @d_address = @d_user.addresses.create!(street: '123 Main St', city: 'Denver', state: 'CO', zip: 80218)
      @admin = User.create(name: 'Sal', email: 'sal@example.com', password: 'securepassword', role: 'admin')
      @admin_address = @admin.addresses.create!(street: '123 Main St', city: 'Denver', state: 'CO', zip: 80218)
    ...
    it 'I see all info a user sees, without edit ability' do
      ...
      expect(page).to have_content(@d_address.street)
      expect(page).to have_content("#{@d_address.city}")
      expect(page).to have_content("#{@d_address.state}")
      expect(page).to have_content("#{@d_address.zip}")
